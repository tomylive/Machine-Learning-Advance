...
Original file is located at
    https://colab.research.google.com/drive/1z00whQTTikoeScU_VF0JwSmNrjV6FeHn
...
Rahap awal yang bertujuan untuk memahami data yang akan digunakan dalam membangun model.
...
"""1. Library Utama
numpy as np: Digunakan untuk operasi matematika, manipulasi array, dan perhitungan numerik. Contoh: membuat array, menghitung mean.
matplotlib.pyplot as plt: Library untuk visualisasi data, digunakan untuk membuat grafik seperti garis, bar, dan scatter plot.
%matplotlib inline: Perintah khusus di Jupyter Notebook agar grafik ditampilkan langsung dalam notebook.
pandas as pd: Library untuk manipulasi dan analisis data berbasis tabel (dataframe), seperti membaca file CSV dan analisis data.
seaborn as sns: Library visualisasi berbasis Matplotlib untuk membuat grafik statistik lebih estetis dan informatif.
warnings.filterwarnings('ignore'): Digunakan untuk menonaktifkan peringatan (warnings) agar output lebih bersih.
2. Model Machine Learning
LogisticRegression: Model untuk klasifikasi linier biner/multikelas berdasarkan probabilitas.
KNeighborsClassifier: Model klasifikasi berbasis metode k-Nearest Neighbors yang mempertimbangkan jarak antar data.
RandomForestClassifier: Algoritme ensemble yang menggunakan beberapa pohon keputusan untuk meningkatkan akurasi prediksi.
DecisionTreeClassifier: Model pohon keputusan yang memprediksi berdasarkan aturan if-else sederhana.
GradientBoostingClassifier: Model ensemble berbasis boosting, menggabungkan model lemah secara bertahap untuk meningkatkan performa.
3. Evaluasi dan Validasi
train_test_split: Membagi dataset menjadi data pelatihan dan pengujian untuk evaluasi performa model.
StratifiedKFold: Teknik cross-validation yang membagi data menjadi beberapa lipatan dengan proporsi kelas yang seimbang.
cross_val_score: Menghitung skor validasi silang untuk model.
4. Evaluasi Kinerja Model
accuracy_score: Menghitung akurasi prediksi (persentase data yang diprediksi benar).
precision_score: Menghitung presisi, yaitu proporsi prediksi positif yang benar.
recall_score: Menghitung sensitivitas, yaitu proporsi kasus positif yang benar terdeteksi.
f1_score: Rata-rata harmonis antara presisi dan sensitivitas.
roc_auc_score: Menghitung skor area under the ROC curve untuk mengukur performa model klasifikasi.
roc_curve: Menghitung koordinat ROC (Receiver Operating Characteristic) untuk membuat kurva.
confusion_matrix: Matriks yang menunjukkan distribusi prediksi model terhadap label sebenarnya (TP, FP, FN, TN).
classification_report: Laporan kinerja model yang mencakup metrik seperti presisi, recall, F1-score, dan support.
5. Hyperparameter Tuning
GridSearchCV: Melakukan pencarian kombinasi terbaik dari hyperparameter model melalui validasi silang.
"""
...
Dataset Kaggle diunggah ulang ke Google Drive pribadi agar lebih mudah diakses tanpa hambatan seperti login. Kolom indeks diatur terpisah agar tidak dianggap sebagai fitur, sehingga hanya data relevan yang digunakan dalam model machine learning.
"""
